...
use crate::game::{
    bomb::resources::HeldBombCount,
    currency::resources::HeldCurrency,
    hud::components::{BombCount, CurrencyText, KillText, ScoreText},
    score::resources::{Kills, Score},
};

pub fn update_score(mut score_query: Query<&mut Text, With<ScoreText>>, score: Res<Score>) {
...
pub fn update_currency(
    ...
    for mut text in &mut currency_query {
        text.sections[0].value =
            "Currency: ".to_owned() + held_currency.amount.to_string().as_str();
    }
}

pub fn update_kill_count(mut kill_query: Query<&mut Text, With<KillText>>, kills: Res<Kills>) {
